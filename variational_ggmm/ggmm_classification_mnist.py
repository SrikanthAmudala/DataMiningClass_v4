...
for train_index, test_index in skf.split(X, y):
    ...
    print("####")
    X_train, X_test = X[train_index]+sys.float_info.epsilon, X[test_index]+sys.float_info.epsilon
    ...
    for dim in range(d):
        ...
        Nk = rnk.sum(axis=0)
        alpha0 = np.asarray([x.mean() ** 2 / (x.var()+sys.float_info.epsilon) for _ in range(k)])
