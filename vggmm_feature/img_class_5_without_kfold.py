...
LOG_FILE_PATH = "/home/k_mathin/PycharmProjects/DataMiningClass/logs/img_class_5_kfold.txt"
    ...
    if sk.any() == 0 or sk.any() == np.nan:
        ...
        try:
            with np.errstate(divide='raise'):
                term2 = 1 / 2 * (rnk * (e_precision_) * ((x.reshape(-1, 1) - mk.reshape(-1, 1).T) ** 2 + 1 / sk)).sum(axis=1)
...
for path in os.listdir(base_path):
    if path in ["MICC_F220_bow_600.csv", "MICC_F220_bow_700.csv","MICC_F220_bow_800.csv"]:
...
#     accuracy_table.append([path, accuracy_list.mean()])
#     totaltime = datetime.datetime.now() - start
#     print("Ttoal: ", totaltime)
# print("Accuracy for all the k's: \n", accuracy_table)
# #