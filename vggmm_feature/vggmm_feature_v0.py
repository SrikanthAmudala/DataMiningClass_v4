...
e_ln_epslon = np.asarray([1, 1])
e_ln_precision_ = digamma(alpha0) - np.log(beta0)

# fsk
# fsnk = np.log(shape[0]) + 1 / 2 * e_ln_precision_ - np.log(
#     2 * gamma(1 / 2)) - e_precision_ * z ** 2 + e_ln_epslon

# fsk
fsnk = np.log(shape[0]) + 1 / 2 * e_ln_precision_ - np.log(
    2 * gamma(1 / 2)) - e_precision_ * np.abs(x.reshape(-1, 1) - mk) ** 2 + e_ln_epslon

# mk
for cluster in range(k):
    for i in range(len(x)):
        if x[i] > mk[cluster]:
            test_mk_num[i, cluster] = rnk[i][cluster] * e_precision_[cluster] * fsnk[i, cluster] * shape[cluster] * abs(
                x[i] ** shape[cluster]) / (x[i] ** 2)

            test_mk_den[i, cluster] = rnk[i][cluster] * e_precision_[cluster] * fsnk[i, cluster] * abs(x[i]) ** shape[
                cluster] * shape[
                                          cluster] * (shape[cluster] - 1) / (2 * x[i] ** 2)
        else:

            test_mk_den[i, cluster] = rnk[i][cluster] * e_precision_[cluster] * fsnk[i, cluster] * mk[cluster] ** (
                    shape[cluster] - 2)
            if test_mk_den[i, cluster] <= 0:
                test_mk_den[i, cluster] = 0

            term0 = rnk[i][cluster] * e_precision_[cluster] * fsnk[i, cluster] * shape[cluster] / 2 * mk[cluster] ** (
                    shape[cluster] - 2) * x[i]

            # remove the -ve sign if necessary
            term1 = rnk[i][cluster] * e_precision_[cluster] * shape[cluster] / 4 * fsnk[i, cluster] * (
                    shape[cluster] - 1) * mk[
...
alphak = fsnk.sum(axis=0) * Nk / 2 + alpha0 - 1
betak = beta0 + fsnk.sum(axis=0) * (rnk * e_x_mean_lamnda_2(mk, x, sk)).sum(axis=0)

e_ln_pi = e_ln_pi_k(gammak, Nk)
e_ln_precision_ = digamma(alphak) - np.log(np.abs(betak))
e_precision_ = alphak / betak
e_x_mean_lambda_ = z.copy()

# fsk
fsnk = np.log(shape[0]) + 1 / 2 * e_ln_precision_ - np.log(
    2 * gamma(1 / 2)) - e_precision_ * e_x_mean_lambda_ ** 2 + e_ln_epslon

e_ln_epslon = fsnk.sum(axis=0) / N
# e_ln_epslon = 1 - e_ln_epslon
...
while (count < 50):
    ...
    for cluster in range(k):
        for i in range(len(x)):
            if x[i] > mk[cluster]:
                test_mk_num[i, cluster] = rnk[i][cluster] * e_precision_[cluster] * fsnk[i, cluster] * shape[
                    cluster] * abs(
                    x[i] ** shape[cluster]) / (x[i] ** 2)

                test_mk_den[i, cluster] = rnk[i][cluster] * e_precision_[cluster] * fsnk[i, cluster] * abs(x[i]) ** \
                                          shape[
                                              cluster] * shape[
                                              cluster] * (shape[cluster] - 1) / (2 * x[i] ** 2)
                ...
                term0 = rnk[i][cluster] * e_precision_[cluster] * fsnk[i, cluster] * shape[cluster] / 2 * mk[
                    cluster] ** (
                                shape[cluster] - 2) * x[i]
        ...
        mk[cluster] = numerator / sk[cluster]
    print(mk, " Shape : ", shape)
    gammak = gama0 + Nk
    e_ln_pi = e_ln_pi_k(gammak, Nk)
    e_ln_precision_ = digamma(alphak) - np.log(np.abs(betak))

    # if betak[0] >0 and betak[1] > 0:
    #     e_ln_precision_ = digamma(alphak) - np.log(betak)
    # else:
    #     e_ln_precision_ = digamma(alphak)
    e_precision_ = alphak / betak
