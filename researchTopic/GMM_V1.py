...
# input_path = "/Users/Srikanth/PycharmProjects/DataMiningClass/datasets/STARfish.png"
...
while sum(sum(np.absolute(np.asarray(means) - np.asarray(meanPrev)))) >= 3:
    ...
    np_resp= np.asarray(resp)
    x_u = np.asarray([feat - means[i] for i in range(k)]).T
    covar_num = np.asarray([np.dot(x_u[:, :, i], x_u[:, :, i].T) for i in range(k)])

    co_num =[]

    for j in range(k):
        co_num.append(np.asarray([np_resp[i, 0] * covar_num[j] for i in range(len(feat))]))

    co_num = np.asarray(co_num)

    co_var = co_num.sum(axis=1)
    co_var = np.asarray([co_var[i]/nK[i] for i in range(k)])
    covariances = co_var

    covariances_Inv = [np.linalg.inv(covariances[i]) for i in range(k)]

    #
    # co_num = np.dot(x_u.T, x_u)
    #
    #
    # co_num = np.asarray([np_resp[i, 0] * co_num for i in range(len(feat))])
    # co_var = co_num.sum(axis=0)/nK[0]
    #
