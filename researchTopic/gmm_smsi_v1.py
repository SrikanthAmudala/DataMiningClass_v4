...
counterr = 0
print("Means: ", means)
print("Mean prev: ", meanPrev)
# iterating until convergence is reached
Ri = S.sum()
saliency_list = list(S)

while sum(sum(np.absolute(np.asarray(means) - np.asarray(meanPrev)))).any() >= 1E-06:
    k = 2
    resp = []
    likelihoods = []
    ...
    print("Mean Num: ", mean_num)
    mean_den = resp_np.sum(axis=0)

    print("Mean Den: ", mean_den)
    new_mean = []

    for i in range(k):
        new_mean.append(mean_num[i] / mean_den[i])
    means = [list(i) for i in new_mean]
    meanPrev = means
    means = new_mean
    # prin
    print("Means: ", means)
    print("Mean Previous: ", meanPrev)
    print("Sum: ", sum(sum(np.absolute(np.asarray(means) - np.asarray(meanPrev)))))
