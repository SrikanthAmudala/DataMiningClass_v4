...
pixels = preprocessing.scale(pixels, axis=0)
img_pixels = pixels.reshape(o_shape)
pyplot.imshow(img_pixels)
# pyplot.show()
# Total no of pixels
N = len(pixels)
...
# iterating until convergence is reached
while sum(sum(np.absolute(np.asarray(means) - np.asarray(meanPrev)))) >= 0.001:
    resp = []
    likelihoods = []
    for feature in feat:
        classLikelihoods = likeli(means, covariances, covariances_Inv, weights, feature, d)
